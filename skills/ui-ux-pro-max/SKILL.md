...
## Agent-agnostic tools (no Cursor / Gemini / Hermes required)

`SKILL_DIR` is the directory that contains this `SKILL.md`.

```bash
SKILL_DIR="$(cd "$(dirname "$0")" && pwd)"   # or: repo/skills/ui-ux-pro-max
python3 "${SKILL_DIR}/scripts/search.py" "dashboard analytics" --domain ux
python3 "${SKILL_DIR}/scripts/search.py" "saas landing fintech" --design-system -p "Example"
```

The search CLI is ordinary Python 3 (stdlib only). Any agent — or a human in a terminal — can run it.
...
The search script lives inside this skill's own directory, not the project directory. Set `SKILL_DIR` to that folder and invoke `scripts/search.py` by full path:
